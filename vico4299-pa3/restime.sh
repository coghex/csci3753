...
echo '\hline'
echo 'Schedule & Utilization & Type & Trial & Response Time\\'
echo '\hline'
echo '\hline'
...
echo -ne $typ
echo -ne ' & 1 & ' 
num=`sudo ./pa3 10 $sched $typ restime`
num=${num%?}
...
for ((i=0; i<9; i++))
do
temp=`sudo ./pa3 10 $sched $typ restime`
...
for ((i=0; i<9; i++))
do
temp=`sudo ./pa3 50 $sched $typ restime`
num=`echo $num`+`echo ${temp%?}`
echo -ne ' & & &' $((i+2)) '& '
...
for ((i=0; i<9; i++))
do
temp=`sudo ./pa3 250 $sched $typ restime`
num=`echo $num`+`echo ${temp%?}`
echo -ne ' & & &' $((i+2)) '& '
